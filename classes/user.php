...
class user extends DbTable
{
	...
	public function __construct()
	{
		...
		// pid
		$this->pid = new DbField('user', 'user', 'x_pid', 'pid', '`pid`', '`pid`', 3, 11, -1, FALSE, '`pid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// username
		$this->username = new DbField('user', 'user', 'x_username', 'username', '`username`', '`username`', 200, 50, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->username->Required = TRUE; // Required field
		$this->username->Sortable = TRUE; // Allow sort
		$this->fields['username'] = &$this->username;

		// password
		$this->password = new DbField('user', 'user', 'x_password', 'password', '`password`', '`password`', 200, 50, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->password->Required = TRUE; // Required field
		$this->password->Sortable = TRUE; // Allow sort
		$this->fields['password'] = &$this->password;

		// userlevel
		$this->userlevel = new DbField('user', 'user', 'x_userlevel', 'userlevel', '`userlevel`', '`userlevel`', 200, 50, -1, FALSE, '`userlevel`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function renderEditRow()
	{
		...
		if (!$this->userlevel->Raw)
			$this->userlevel->CurrentValue = HtmlDecode($this->userlevel->CurrentValue);
	...
	public function getUserIDFilter($userId)
	{
		...
		$parentUserIdFilter = '`id` IN (SELECT `id` FROM ' . "`user`" . ' WHERE `pid` = ' . QuotedValue($userId, DATATYPE_NUMBER, Config("USER_TABLE_DBID")) . ')';
