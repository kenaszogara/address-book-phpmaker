...
class AdvancedSecurity
{
	...
	public function loadUserID()
	{
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if (Config("USER_ID_IS_HIERARCHICAL")) {
	...
	public function parentUserIDList($userId)
	{
		...
		if (SameString($userId, CurrentUserID())) {
			if (strval(CurrentParentUserID()) != "")
				$result = QuotedValue(CurrentParentUserID(), DATATYPE_NUMBER, Config("USER_TABLE_DBID"));
			return $result;
		}

		// One level only, must be CurrentUserID
		if (!Config("USER_ID_IS_HIERARCHICAL")) {
			return QuotedValue(CurrentUserID(), DATATYPE_NUMBER, Config("USER_TABLE_DBID"));
			...
			for ($i = 0; $i < $len; $i++) {
				if (strval($ar[$i]) != trim(strval($userId))) {
					if ($result != "")
						$result .= ", ";
					$result .= QuotedValue($ar[$i], DATATYPE_NUMBER, Config("USER_TABLE_DBID"));
				}
			}
			return $result;
		}
